...
const Gamma = () => {
  ...
  const calculateStats = () => {
    ...
    for (let i = 0; i < wineData.length; i++) {
      if (wineData[i].Alcohol === 1) {
        let gamma = parseFloat(
          (wineData[i].Ash * wineData[i].Hue) / wineData[i].Magnesium
        );
        meanc1 += gamma;
        countC1++;
        arrC1.push(gamma);
    ...
    let medc2 = median(arrC2).toFixed(3);
    let medc3 = median(arrC3).toFixed(3);

    let modec1 = mode(arrC1).toFixed(3);
    let modec2 = mode(arrC2).toFixed(3);
    let modec3 = mode(arrC3).toFixed(3);
  ...
  return (
    <div className="container">
      <h2 style={{ paddingLeft: "80px" }}>Gamma : Mean, Median and Mode</h2>
      <table>
        <thead>
          <tr>
            <th>Measure</th>
